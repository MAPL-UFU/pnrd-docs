---
id: pnrd-core-arduino
title: PNRD Arduino Core Concepts
---

This guide describes how to use the MAPL command line interface (CLI) client to interact with the MAPL Blockchain. The CLI is invoked as an interactive shell. It provides basic commands to create accounts, mint coins, perform transfers, and query the blockchain. You can use the CLI client to interact with a validator node on the testnet, on a local MAPL Blockchain, or on a remote blockchain by specifying the node's hostname.

## Invocation
There are three ways to invoke the MAPL CLI client.
...
### Run a Local MAPL Network and Spawn a CLI Client
To start a local MAPL network and spawn a CLI client that connects to this local network, run:
```bash
cargo run -p libra_swarm -- -s

```
The `-s` option causes the CLI to be run after the local MAPL network is launched.  Note that this may take a few minutes to build and then start.

### Run a CLI Client to Connect to Any MAPL Network
...
The options for running the CLI client command to connect to any MAPL network are:
...
* `-p | -—port` &mdash; The public port of the destination MAPL Blockchain for the client to connect with, the default value is 30307.
...
---
 ...
 #### `account | a` &mdash;  Account related operations. Subcommands include:
  ...
  `write | w <file path>` &mdash; Save MAPL wallet mnemonic recovery seed to disk.  This will allow accounts to be recovered via `account recover`.
